...
VF_RETURN	VF_Process()
{
	...
	// 计算方向场
	getOrientMap(6);
	saveBitmap(g_lpOrient, IMGW, IMGH, (sint8*)"/tmp/0.bmp");
	...
	saveBitmap(g_lpDivide, IMGW, IMGH, (sint8*)"/tmp/00.bmp");

	// 两次定向滤波
	orientFilter();
//	orientFilter();
	saveBitmap(g_lpOrgFinger, IMGW, IMGH, (sint8*)"/tmp/1.bmp");

	// 二值化
	binary();
	saveBitmap(g_lpOrgFinger, IMGW, IMGH, (sint8*)"/tmp/2.bmp");

	// 二值化图像去噪声
	binaryClear();
	saveBitmap(g_lpOrgFinger, IMGW, IMGH, (sint8*)"/tmp/3.bmp");

	// 清除背景
	clearEdge();
	saveBitmap(g_lpOrgFinger, IMGW, IMGH, (sint8*)"/tmp/4.bmp");

	// 细化
	thin();
	// 细化图像去短棒和毛刺
	thinClear(12);
	saveBitmap(g_lpOrgFinger, IMGW, IMGH, (sint8*)"/tmp/5.bmp");
...
///////////////////////////////////////////////////////////////////////
	...
	if (lpMatchResult->Similarity >= CFG_MATCH_SCORE_LEVEL)//原来为50
		return TRUE;
	else
		return FALSE;
}
